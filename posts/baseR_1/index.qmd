...
```{r}
packages = installed.packages()[,c(1,4)] |>
...
funs = Filter(is.function, sapply(ls(baseenv(), all.names=TRUE), get, baseenv()))
length(funs)
```
...
```{r}
grep("print.", baseenv() |> ls(all.names=TRUE), value=TRUE, fixed=TRUE) |> length()
```
